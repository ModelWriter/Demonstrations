...
//fact { Requirement - Model.mandatory = Model.optional }

/** Converts the feature model into a requirements model */
fact Convert {
	let fm = FeatureModel/FeatureModel {
		...
		//Model.mandatory = fm.root.*(fm.mandatory + fm.mandatoryOr)
	}
}

assert semantically_equals {
	let fm = FeatureModel/GivenFeatureModel, rm = RequirementsModel/GivenModel {
		...
		all f: Mandatory | Root in f.~*(fm.(mandatoryOr + mandatory)) || f = FeatureModel/Root
		//all f: Model.optional | some f.~*(fm.(alternative + optionalOr + optional))
		//all f: Model.mandatory | some f.~*(fm.(mandatoryOr + mandatory)) || f = fm.root
	}
}
